...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const activePage = document.querySelector('.admin-menu-item.active').getAttribute('data-page');
    if (activePage === 'donghua') {
        loadDonghuaTable();
    } else if (activePage === 'episode') {
        loadEpisodeTable();
        loadDonghuaOptions();
...
function setupSidebar() {
    ...
    menuItems.forEach(item => {
        ...
        item.addEventListener('click', function() {
            ...
            // Load data if needed
            if (page === 'donghua' && !document.querySelector('#donghuaTableBody tr')) {
                loadDonghuaTable();
            } else if (page === 'episode' && !document.querySelector('#episodeTableBody tr')) {
                loadEpisodeTable();
                loadDonghuaOptions();
            } else if (page === 'users' && !document.querySelector('#usersTableBody tr')) {
                loadUsersTable();
            }
...
// Setup logout button specifically
function setupLogoutButton() {
    ...
    if (logoutBtn) {
        ...
        logoutBtn.addEventListener('click', async function(e) {
            ...
            e.preventDefault();
            try {
                // Call the logoutUser function from auth.js
                await logoutUser();
                console.log('Logout successful');
                // Redirect should happen in logoutUser but just in case:
                window.location.href = 'login-admin.html';
            } catch (error) {
